...
import VitoCorleone from './assets/images/Vito Corleone.jpg';
import CarmeliaCorleone from './assets/images/Carmelia Corleone.jpg';
import CarloRizzi from './assets/images/Carlo Rizzi.jpg';
import LuciMancini from './assets/images/Luci Mancini.jpg';
...
class App extends Component {
    ...
    selectEmployee = (event, name) => {
        event.preventDefault();
        const employeeCopy = JSON.parse(JSON.stringify(this.state.employees));
        employeeCopy.forEach(employee => {
            employee.selected = false;
        });

        const employeeIndex = employeeCopy.findIndex(employee => employee.name === name);

        employeeCopy[employeeIndex].selected = true;

        this.setState(prevState => ({
            ...prevState,
            employees: employeeCopy,
            selectedEmployee: employeeCopy[employeeIndex]
        }));
    };

    slideChange = x => {
        const selectedEmployeeCopy = JSON.parse(JSON.stringify(this.state.selectedEmployee));
        selectedEmployeeCopy.popularity = x;

        const employeesCopy = JSON.parse(JSON.stringify(this.state.employees));
        const employeeIndex = employeesCopy.findIndex(employee => employee.name === selectedEmployeeCopy.name);

        employeesCopy[employeeIndex].popularity = x;

        this.setState(prevState => ({
            ...prevState,
            employees: employeesCopy,
            selectedEmployee: selectedEmployeeCopy
        }));
    };
            ...
            <div className="app">
                <div className={'header-image'}>
                    ...
                    <div className={'content-container'}>
                        <Menu {...this.state} selectEmployee={this.selectEmployee}/>
                        <SelectedEmployee selectedEmployee={this.state.selectedEmployee}
